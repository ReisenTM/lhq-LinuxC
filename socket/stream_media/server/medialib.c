...
// 将某个目录下的所有文件转为一个频道(分析路径保存到一个结构体中)
static struct channel_context_st *path2entry(const char *path) {
  ...
  me->chnid = curr_id;
  curr_id++; // 将 curr_id 增加 1，以便下一个频道使用不同的 ID。
  return me;
}

// 确定媒体库位置
int mlisb_getchnlist(struct mlib_listinfo_t **result, int *resnum) {
  char path[PATHSIZE];
  glob_t globres;
  int num = 0;
  struct mlib_listinfo_t *ptr;
  ...
  ptr = malloc(sizeof(struct mlib_listinfo_t) * globres.gl_pathc);
  if (ptr == NULL) {
    syslog(LOG_ERR, "malloc() default");
    exit(EXIT_FAILURE);
  }
#ifdef DEBUG
  printf("globres 0:%s\n", globres.gl_pathv[0]);
  ...
  *result = realloc(ptr, sizeof(struct mlib_listinfo_t) * num);
  if (*result == NULL) {
...
ssize_t mlib_readchn(chnid_t chnid, void *buf, size_t size) {
  ...
  while (1) {
    len = pread(channel[chnid].fd, buf, tbfsize,
                channel[chnid].offset); // 读取tbfsize数据到从offset处开始的buf
    ...
    } else /*len > 0*/ // 真正读取到了数据
    {
      ...
      syslog(LOG_DEBUG, "epoch : %f%%",
             (channel[chnid].offset) / (1.0 * buf.st_size) *
                 100); // 实现类似播放进度的效果
      break;
    }
  }
...
int mlib_freechnlist(struct mlib_listinfo_t *ptr) {
  free(ptr);
  return 0;
}
