...
static struct mlib_listinfo_t *list_entry; // 频道列表

static void *thr_list(void *p) {
  int totalsize;
  struct msg_list_st *entrylistptr;
  struct msg_listinfo_t *entryptr;
  int ret;
  int size;

  totalsize = sizeof(chnid_t); // 之后逐步累计
  for (int i = 0; i < num_list_entry; ++i) {
    totalsize += sizeof(struct msg_listinfo_t) + strlen(list_entry[i].desc);
  }
  ...
  entrylistptr->chnid = CHNLIST; // 这是列表频道

  entryptr = entrylistptr->list;
  syslog(LOG_DEBUG, "nr_list_entn:%d\n", num_list_entry);
  for (int i = 0; i < num_list_entry; ++i) {
    size = sizeof(struct msg_listinfo_t) + strlen(list_entry[i].desc);
  ...
  while (1) {
    ...
    ret = sendto(serverfd, entrylistptr, totalsize, 0, (void *)&sndaddr,
                 sizeof(sndaddr)); // 频道列表在广播网段每秒发送entrylist
    syslog(LOG_DEBUG, "sent content len:%d\n", entrylistptr->list->len);
    if (ret < 0) {
...
// 创建节目单线程
int thr_list_create(struct mlib_listinfo_t *listptr, int num_ent) {
  int err;
  list_entry = listptr;
